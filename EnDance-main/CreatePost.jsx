...
import styles from './app/create-post/CreatePost.module.css';
...
export default function CreatePost() {
  ...
  const locations = ['Vancouver', 'Coquitlam', 'Kelowna', 'Burnaby', 'Port Moody'];
  ...
  if (showSplash) {
    return <PostCreatedSplash onContinue={() => { /* Add navigation logic here */ }} />;
  }
  ...
  return (
    <div className="create-post-container">
      <div className="create-post-content">
        ...
        <div className="header-row">
          <div className="back-button-container">
            <svg width="14" height="23" viewBox="0 0 14 23" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M11.7109 2.29529L2.62695 11.5905L11.7109 20.8857" stroke="black" strokeWidth="3.09841" strokeLinecap="round"/>
            </svg>
          </div>
          <h2 className="create-post-header heading-main">Create Post</h2>
        </div>
        ...
        <div className="create-captions-title heading-main">Create Captions</div>
        <TextArea placeholder="Type here..." />

        {/* Location filter dropdown */}
        <Filter
          icon={locationIcon}
          label={selectedLocation ? <span className="filter-label">{selectedLocation}</span> : <span className="filter-label">Add Location</span>}
          dropdown
        >
        ...
        <Filter
          icon={danceIcon}
          label={selectedDance ? <span className="filter-label">{selectedDance}</span> : <span className="filter-label">Dance Style</span>}
          dropdown
        >
          <div className={styles.dropdown}>
            {danceStyles.map((style) => (
              <div
                key={style}
                className={`${styles.dropdownItem}${selectedDance === style ? ' ' + styles.selected : ''}`}
